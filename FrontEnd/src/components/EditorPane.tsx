...
import { ReactFlow, Background, Controls, MiniMap, useNodesState, useEdgesState, addEdge, useReactFlow, NodeTypes } from '@xyflow/react';
import '@xyflow/react/dist/style.css'; // Core styles
import { motion } from 'framer-motion'; // Anims
import Sidebar from './Sidebar'; // Node palette
import type { WorkflowNode, Workflow } from '../types'; // Types
...
const EditorPane: React.FC<EditorPaneProps> = memo(({ workflow, onSave, onTest }) => {
  ...
  const [nodes, setNodes, onNodesChange] = useNodesState(workflow.nodes.map(node => ({ ...node, type: node.type, data: { ...node.data, label: node.name } })));
  const [edges, setEdges, onEdgesChange] = useEdgesState(workflow.connections.map(conn => ({ id: `${conn.from}-${conn.to}`, source: conn.from, target: conn.to })));
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-screen flex bg-gray-100 dark:bg-gray-900">
      <Sidebar onDragStart={(event, nodeType) => reactFlowInstance.addNode({ type: nodeType, position: { x: 0, y: 0 } })} /> {/* Draggable palette */}
      <div className="flex-1 relative">
        <ReactFlow
          ...
          nodeTypes={CustomNodes as NodeTypes}
          fitView
